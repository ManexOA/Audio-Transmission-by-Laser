...
byte theByte = 0;  //Define the variable to store the sound data that receives the sensor
                  // We select the Pin 0, which is RX. This is used to receive the serial port data.
...
ISR(TIMER1_COMPA_vect) {
        ...
        if(speakerPin==11){
            OCR2A = theByte; //pgm_read_byte(&sounddata_data[sample])
        } else {
...
void setup()
{
  Serial.begin(80000);  // Initialize the serial port
                       // This determines how many bits per second transmits the serial bus
                       // which also determines the speed reproduction of the sound 
...
void loop(){
    while (Serial.available()){  // Keep looping while data is sending from the serial port
     theByte = Serial.read();   //Read the data coming from the serial port.. 
                               //..and store in the variable "theByte"
    }
}
